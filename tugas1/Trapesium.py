...
class FrmTrapesium:
    def __init__(self, parent, title):
        self.parent = parent
        self.parent.geometry("300x270")
        self.parent.title(title)
        self.parent.protocol("WM_DELETE_WINDOW", self.onKeluar)
        self.aturKomponen()
        
    def aturKomponen(self):
        Label(root, text="Menghitung Luas & Keliling Belah Ketupat").pack()
        ...
        self.txtsisiM.grid(row=3, column=1, padx=5, pady=5)
        self.txtLuas = Entry(mainFrame)
        self.txtLuas.grid(row=5, column=1, padx=5, pady=5)
        self.txtKel = Entry(mainFrame)
        self.txtKel.grid(row=6, column=1, padx=5, pady=5)
...
if __name__ == '__main__':
    root = Tk()
    aplikasi = FrmTrapesium(root, "By. Alfa Nashiha")
    root.mainloop() 